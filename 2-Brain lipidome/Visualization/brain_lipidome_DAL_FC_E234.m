%% brain_lipid_7_FC_rawadb_species_anoava_allE234.m
%%%%% FC heatmap 
%%%%
close all hidden;
clear;
...
%%
mat_path = 'D:\Cotical lipidome\Qiao-analysis\ana_mat';
cd(mat_path);
load('CRAL-brain-lipid-species-anova-rawabd_modify0.mat');
save_fig_path = 'D:\Cotical lipidome\Qiao-analysis\ana_fig\ALCR-rawabd-log-modify0-10-6';
p_thr = 0.05;
...
%% E2 0 DAL E3 2 DAL E4 3 DAL
cd(mat_path);
load CRAL-brain-lipid-species-anova-E2-sex-diet.mat;
...
load CRAL-brain-lipid-species-anova-E3-sex-diet.mat;
...
load CRAL-brain-lipid-species-anova-E4-sex-diet.mat;
...
%%% costum list 4 DAL inlcudes the DALs in E3 and E4.
custom_DAL_list = unique([species_sige2;species_sige3;species_sige4]);
bofp = [bof_pe2,bof_pe3,bof_pe4];

%% expression hierarichical heatmap
cd(mat_path);
load brain_lipidome.mat;
lipidabd = brain_lipid.lipid_level;
mouseinfo = brain_lipid.mouse_info;
mouse_id = brain_lipid.lipidome_id1;
lipid = brain_lipid.lipid_list;
...
cd(mat_path);
load brain_lipidome.mat;
lipidlist = brain_lipid.lipid_list;
fc = brain_lipid.fc;
fce2 = brain_lipid.fce2;
fce3 = brain_lipid.fce3;
fce4 = brain_lipid.fce4;

fccmb = [fce2,fce3,fce4];
%% fc map; bof p map
[idt0,ida0,idb0] = intersect(y_label,lipidlist,'stable');
fc_sigs = fccmb(idb0,:);
[idt00,ida00,idb00] = intersect(y_label,species,'stable');
bofp_sigs = bofp(idb00,:);
